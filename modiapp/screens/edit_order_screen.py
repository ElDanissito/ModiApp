...
class EditOrderScreen(QWidget):
    order_updated = Signal()  # Signal to notify when an order is updated
    ...
    def __init__(self, db, order_id):
        ...
        if not self.order_data:
            QMessageBox.critical(self, "Error", "No se pudo cargar la orden")
            self.close()
            return
            
        self.setWindowTitle("Editar Orden")
        self.setObjectName("editOrderScreen")
        self.resize(1200, 800)
        
        # Get order number from existing data
        self.order_number = self.order_data['order']['order_number']
    ...
    def update_order(self):
        """Update the existing order with current form data"""
        try:
            # Collect order data
            order_data = {
                ...
                'order_value': int(float(self.valor_orden_edit.text())) if self.valor_orden_edit.text() else 0,
                'deposit': int(float(self.abono_edit.text())) if self.abono_edit.text() else 0,
            }
            ...
            for row in self.ref_rows:
                ...
                if ref_edit.text() and color_edit.text() and valor_edit.text():
                    references.append({
                        ...
                        'value': int(float(valor_edit.text()))
                    })
            ...
            # Update in database
            if self.db.update_order(self.order_id, db_data):
                QMessageBox.information(self, "Éxito", "Orden actualizada correctamente")
                self.order_updated.emit()
                self.close()
            else:
                QMessageBox.critical(self, "Error", "Error al actualizar la orden")

        except Exception as e:
            QMessageBox.critical(self, "Error", f"Error al actualizar la orden: {str(e)}")
            import traceback
            traceback.print_exc()

    def load_order_data(self):
        """Load existing order data into the form fields"""
        try:
            order = self.order_data['order']
            details = self.order_data['details']
            references = self.order_data['references']

            # Load basic order information
            self.cliente_name_edit.setText(order.get('client_name', ''))
            self.fecha_orden_edit.setDate(QDate.fromString(order.get('order_date', ''), "yyyy-MM-dd"))
            self.fecha_entrega_edit.setDate(QDate.fromString(order.get('delivery_date', ''), "yyyy-MM-dd"))
            
            # Load financial information
            self.valor_orden_edit.setText(str(order.get('order_value', 0)))
            self.abono_edit.setText(str(order.get('deposit', 0)))
            self.update_saldo()

            # Load client details
            if 'orden' in details:
                orden_details = details['orden']
                self.direccion_edit.setText(orden_details.get('Direccion', ''))
                self.telefono_edit.setText(orden_details.get('Telefono', ''))
                self.cc_edit.setText(orden_details.get('CC', ''))
                
                # Load SM, SC, AG, AC
                for field in ['SM', 'SC', 'AG', 'AC']:
                    if field in self.sm_sc_edits:
                        self.sm_sc_edits[field].setText(orden_details.get(field, ''))
            ...
            if 'camisa' in details:
                self.load_camisa_details(details['camisa'])

            # Load saco details
            if 'saco' in details:
                self.load_saco_details(details['saco'])

            # Load pantalon details
            if 'pantalon' in details:
                self.load_pantalon_details(details['pantalon'])
    ...
    def load_references(self, references):
        ...
        for ref in references:
            row = self.add_reference_row()
            ref_edit = row.layout().itemAt(0).widget()
            color_edit = row.layout().itemAt(1).widget()
            valor_edit = row.layout().itemAt(2).widget()
            
            ref_edit.setText(ref.get('reference', ''))
            color_edit.setText(ref.get('color', ''))
            valor_edit.setText(str(ref.get('value', 0)))

        # Ensure at least 2 rows exist
        while len(self.ref_rows) < 2:
            self.add_reference_row()

    def load_camisa_details(self, camisa_data):
        """Load camisa details into form fields"""
        # Load measurements
        for field, edit in self.camisa_measure_edits.items():
            if field in camisa_data and camisa_data[field]:
                edit.setText(str(camisa_data[field]))

        # Load modelo espalda
        if 'modelo_espalda' in camisa_data:
            self.select_radio_button_by_text(self.espalda_button_group, camisa_data['modelo_espalda'])

        # Load prespuente
        if 'modelo_espalda_prespuente' in camisa_data:
            index = self.prespuente_combo.findText(camisa_data['modelo_espalda_prespuente'])
            if index >= 0:
                self.prespuente_combo.setCurrentIndex(index)

        # Load checkboxes
        if 'modelo_espalda_pechera' in camisa_data:
            self.pechera_checkbox.setChecked(camisa_data['modelo_espalda_pechera'] == 'True')
        if 'modelo_espalda_tapa_boton' in camisa_data:
            self.tapa_boton_checkbox.setChecked(camisa_data['modelo_espalda_tapa_boton'] == 'True')

        # Load modelo bolsillo
        if 'modelo_bolsillo' in camisa_data:
            self.select_radio_button_by_text(self.modelo_bolsillo_button_group, camisa_data['modelo_bolsillo'])
            if camisa_data['modelo_bolsillo'] != 'NO':
                if 'lado_bolsillo' in camisa_data:
                    index = self.lado_bolsillo_combo.findText(camisa_data['lado_bolsillo'])
                    if index >= 0:
                        self.lado_bolsillo_combo.setCurrentIndex(index)
                if 'cantidad_bolsillo' in camisa_data:
                    self.cantidad_bolsillo_input.setText(str(camisa_data['cantidad_bolsillo']))

        # Load modelo puño
        if 'modelo_puno' in camisa_data:
            self.select_radio_button_by_text(self.modelo_puno_button_group, camisa_data['modelo_puno'])
        if 'textura_puno' in camisa_data:
            self.select_radio_button_by_text(self.textura_puno_button_group, camisa_data['textura_puno'])
        if 'ancho_cms_puno' in camisa_data:
            self.ancho_input.setText(str(camisa_data['ancho_cms_puno']))

        # Load modelo cuello
        if 'modelo_cuello' in camisa_data:
            if camisa_data['modelo_cuello'] not in ['Pegasso', 'Valentino C', 'Crown', 'Givenchy', 'Pajarito', 'Valentino L', 'Neru', 'Royal']:
                self.rb_otro_cuello.setChecked(True)
                self.otro_cuello_input.setText(camisa_data['modelo_cuello'])
            else:
                self.select_radio_button_by_text(self.modelo_cuello_button_group, camisa_data['modelo_cuello'])

        if 'textura_cuello' in camisa_data:
            self.select_radio_button_by_text(self.textura_cuello_button_group, camisa_data['textura_cuello'])
        if 'plum_cuello' in camisa_data:
            self.select_radio_button_by_text(self.plum_cuello_group, camisa_data['plum_cuello'])
        if 'bottom_down_cuello' in camisa_data:
            self.select_radio_button_by_text(self.bd_cuello_group, camisa_data['bottom_down_cuello'])
        ...
        # Load prenda and empaque
        if 'Prenda' in camisa_data:
            self.select_radio_button_by_text(self.prenda_group, camisa_data['Prenda'])
        if 'Empaque' in camisa_data:
            self.select_radio_button_by_text(self.empaque_group, camisa_data['Empaque'])

        # Load contextura fisica
        if 'contextura_fisica' in camisa_data:
            self.select_radio_button_by_text(self.contextura_group, camisa_data['contextura_fisica'])

    def load_saco_details(self, saco_data):
        """Load saco details into form fields"""
        # Load measurements
        for field, edit in self.saco_measure_edits.items():
            if field in saco_data and saco_data[field]:
                edit.setText(str(saco_data[field]))

        # Load estilo
        if 'Estilo' in saco_data:
            self.select_radio_button_by_text(self.estilo_saco_group, saco_data['Estilo'])
        if 'cantidad_boton' in saco_data:
            self.cant_boton_edit.setText(str(saco_data['cantidad_boton']))

        # Load solapa
        if 'Solapa' in saco_data:
            self.select_radio_button_by_text(self.solapa_saco_group, saco_data['Solapa'])
        if 'ojal_solapa' in saco_data:
            self.ojal_solapa_checkbox.setChecked(saco_data['ojal_solapa'] == 'True')

        # Load bolsillo inferior
        if 'Bolsillo Inferior 1' in saco_data:
            self.select_checkbox_by_text(self.bolsillo_inf_checkboxes, saco_data['Bolsillo Inferior 1'])
        if 'Bolsillo Inferior 2' in saco_data:
            self.select_checkbox_by_text(self.bolsillo_inf_checkboxes, saco_data['Bolsillo Inferior 2'])

        # Load bolsillo superior
        if 'bolsillo_superior' in saco_data and saco_data['bolsillo_superior'] != 'NO':
            # Find and check the SI radio button
            for button in self.bolsillo_sup_enable_group.buttons():
                if button.text() == 'SI':
                    button.setChecked(True)
                    break
            # Select the type
            if 'bolsillo_superior' in saco_data:
                self.select_radio_button_by_text(self.bolsillo_sup_type_group, saco_data['bolsillo_superior'])
        else:
            # Find and check the NO radio button
            for button in self.bolsillo_sup_enable_group.buttons():
                if button.text() == 'NO':
                    button.setChecked(True)
                    break

        # Load delantero and abertura
        if 'Delantero' in saco_data:
            self.select_radio_button_by_text(self.delantero_group, saco_data['Delantero'])
        if 'Abertura' in saco_data:
            self.select_radio_button_by_text(self.abertura_group, saco_data['Abertura'])

        # Load chaleco
        if 'Modelo chaleco' in saco_data and saco_data['Modelo chaleco'] == 'SI':
            # Find and check the SI radio button
            for button in self.chaleco_enable_group.buttons():
                if button.text() == 'SI':
                    button.setChecked(True)
                    break
            # Load chaleco measurements
            if 'chaleco_diagonal_pecho' in saco_data:
                self.diagonal_pecho_edit.setText(str(saco_data['chaleco_diagonal_pecho']))
            if 'chaleco_centro' in saco_data:
                self.centro_edit.setText(str(saco_data['chaleco_centro']))
            if 'chaleco_largo_espalda' in saco_data:
                self.largo_espalda_edit.setText(str(saco_data['chaleco_largo_espalda']))
            if 'observaciones_chaleco' in saco_data:
                self.obs_chaleco_edit.setPlainText(str(saco_data['observaciones_chaleco']))

        # Load observaciones and vendedor
        if 'observaciones' in saco_data:
            self.saco_observaciones_edit.setPlainText(str(saco_data['observaciones']))
        if 'vendedor' in saco_data:
            self.saco_vendedor_edit.setText(str(saco_data['vendedor']))

    def load_pantalon_details(self, pantalon_data):
        """Load pantalon details into form fields"""
        # Load measurements
        for field, edit in self.pantalon_measure_edits.items():
            field_key = field.replace('_', ' ').upper()
            if field_key in pantalon_data and pantalon_data[field_key]:
                edit.setText(str(pantalon_data[field_key]))

        # Load bolsillo delantero
        if 'Bolsillo Delantero' in pantalon_data:
            self.select_radio_button_by_text(self.bolsillo_del_group, pantalon_data['Bolsillo Delantero'])

        # Load bolsillo trasero
        if 'Bolsillo Trasero' in pantalon_data:
            self.select_radio_button_by_text(self.bolsillo_tras_group, pantalon_data['Bolsillo Trasero'])

        # Load terminado bolsillo trasero
        if 'terminado_bolsillo_trasero_boton' in pantalon_data:
            self.select_radio_button_by_text(self.terminado_boton_group, pantalon_data['terminado_bolsillo_trasero_boton'])
        if 'terminado_bolsillo_trasero_oreja' in pantalon_data:
            self.select_radio_button_by_text(self.terminado_oreja_group, pantalon_data['terminado_bolsillo_trasero_oreja'])
        if 'terminado_bolsillo_trasero_parche' in pantalon_data:
            self.terminado_parche_checkbox.setChecked(pantalon_data['terminado_bolsillo_trasero_parche'] == 'True')
            if pantalon_data['terminado_bolsillo_trasero_parche'] == 'True' and 'terminado_bolsillo_trasero_parche_mod' in pantalon_data:
                self.terminado_parche_mod_edit.setText(str(pantalon_data['terminado_bolsillo_trasero_parche_mod']))

    def select_radio_button_by_text(self, button_group, text):
        """Helper method to select a radio button by its text"""
        for button in button_group.buttons():
            if button.text() == text:
                button.setChecked(True)
                break

    def select_checkbox_by_text(self, checkbox_dict, text):
        """Helper method to select a checkbox by its text"""
    ...
    def init_ui(self):
        """Initialize the user interface - identical to CreateOrderScreen"""
        ...
        # Add save button
        save_button = QPushButton("Actualizar Orden")
        save_button.clicked.connect(self.update_order)
    ...
    def update_order_total(self):
        ...
        for row in self.ref_rows:
            ...
            if valor_edit.text():
                try:
                    total += float(valor_edit.text())
                except ValueError:
                    # Si hay error en la conversión, ignorar ese valor
                    continue
        
        self.valor_orden_edit.setText(str(int(total)))
        self.update_saldo()

    def update_saldo(self):
        try:
            valor_text = self.valor_orden_edit.text()
            abono_text = self.abono_edit.text()
            
            # Convertir a float primero para manejar valores decimales
            valor = float(valor_text) if valor_text else 0.0
            abono = float(abono_text) if abono_text else 0.0
            
            saldo = valor - abono
            self.saldo_label.setText(f"$ {saldo:,.0f}")
        except ValueError:
            # Si hay error en la conversión, mostrar 0
            self.saldo_label.setText("$ 0")

    def apply_styles(self):
        """Aplicar estilos CSS a la pantalla de edición de órdenes"""
...
class QIntValidator(QValidator):
    ...
    def fixup(self, input):
        ...
        if 'contextura_fisica_espalda' in camisa_data:
            self.contextura_espalda_edit.setText(str(camisa_data['contextura_fisica_espalda']))
        if 'contextura_fisica_abdomen' in camisa_data:
            self.contextura_abdomen_edit.setText(str(camisa_data['contextura_fisica_abdomen']))

        # Load observaciones and vendedor
        if 'observaciones' in camisa_data:
            self.camisa_observaciones_edit.setPlainText(str(camisa_data['observaciones']))
        if 'vendedor' in camisa_data:
            self.camisa_vendedor_edit.setText(str(camisa_data['vendedor']))

        # Load pretina
        if 'Pretina' in pantalon_data:
            self.select_radio_button_by_text(self.pretina_modelos_group, pantalon_data['Pretina'])
        if 'Boton' in pantalon_data:
            self.pretina_boton_checkbox.setChecked(pantalon_data['Boton'] == 'True')
        if 'Gancho' in pantalon_data:
            self.pretina_gancho_checkbox.setChecked(pantalon_data['Gancho'] == 'True')
        if 'Pasadores' in pantalon_data:
            self.select_radio_button_by_text(self.pasadores_group, pantalon_data['Pasadores'])

        # Load forrado
        if 'forrado' in pantalon_data and pantalon_data['forrado'] == 'SI':
            # Find and check the SI radio button
            for button in self.forrado_enable_group.buttons():
                if button.text() == 'SI':
                    button.setChecked(True)
                    break
            # Load forrado options
            if 'forrado_opcion_1' in pantalon_data:
                self.select_radio_button_by_text(self.forrado_opcion_1_group, pantalon_data['forrado_opcion_1'])
            if 'forrado_opcion_2' in pantalon_data:
                self.select_radio_button_by_text(self.forrado_opcion_2_group, pantalon_data['forrado_opcion_2'])

        # Load estilo delantero
        if 'estilo_delantero' in pantalon_data:
            self.select_radio_button_by_text(self.estilo_delantero_group, pantalon_data['estilo_delantero'])

        # Load other fields
        if 'Especial' in pantalon_data:
            self.pantalon_especial_edit.setText(str(pantalon_data['Especial']))
        if 'Relojera' in pantalon_data:
            self.pantalon_relojera_checkbox.setChecked(pantalon_data['Relojera'] == 'True')
        if 'Bota' in pantalon_data:
            self.select_radio_button_by_text(self.bota_group, pantalon_data['Bota'])

        # Load observaciones and vendedor
        if 'observaciones' in pantalon_data:
            self.pantalon_observaciones_edit.setPlainText(str(pantalon_data['observaciones']))
        if 'vendedor' in pantalon_data:
            self.pantalon_vendedor_edit.setText(str(pantalon_data['vendedor']))
